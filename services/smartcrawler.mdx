...
try {
  const response = await smartCrawler(apiKey, url, prompt, depth, maxPages, null, rules);
...
| include_paths | list   | []      | List of path patterns to include (e.g., `["/products/*", "/blog/**"]`). Supports wildcards: `*` matches any characters, `**` matches any path segments. If empty, all paths are included. |
| exclude_paths | list   | []      | List of path patterns to exclude (e.g., `["/admin/*", "/api/**"]`). Supports wildcards: `*` matches any characters, `**` matches any path segments. Takes precedence over `include_paths`. |
...
const response = await smartCrawler(apiKey, url, prompt, 1, 5, CompanySchema, rules);
```
