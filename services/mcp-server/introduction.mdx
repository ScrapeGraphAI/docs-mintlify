...
### OAuth login with Google (recommended)

Add the server without an API key. The client will use OAuth to open the ScrapeGraphAI login page:
...
<Steps>
  ...
  <Step title="Start authentication">
    Run <code>/mcp</code> in your client, select <code>sgai</code>, and choose the option to authenticate. In Codex or Claude Code, you can also run <code>codex mcp login sgai</code> or <code>claude mcp login sgai</code> from the terminal.
  </Step>
...
To test the OAuth connection, list the configured servers with `codex mcp get sgai` or `claude mcp get sgai`, then ask the client to run the `credits` tool. A successful response confirms that OAuth authentication and MCP tool calls are both working.
...
- Read the detailed setup guide for <a href="/services/mcp-server/claude-code">Claude Code</a>
- Read the detailed setup guide for <a href="/services/mcp-server/codex">Codex</a>
