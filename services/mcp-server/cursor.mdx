...
Restart Cursor, open its MCP settings, select `sgai`, and start the OAuth login. In the browser window, choose **Continue with Google** and sign in with the Google account associated with your ScrapeGraphAI account.

Once the browser shows **Authentication successful. Connected to sgai.**, return to Cursor and start using the tools.

Test the connection by asking Cursor to run the `credits` tool.
