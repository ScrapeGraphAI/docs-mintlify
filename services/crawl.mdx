...
A Crawl job costs **2 credits** to start, plus the per-page Scrape cost for every page processed (1 credit for `markdown`, 2 for `screenshot`, 25 for `branding`; multiple formats are summed). Enabling `stealth` in `fetchConfig` adds 5 credits per page; render mode (`auto` / `fast` / `js`) does not affect the cost. See the [pricing page](https://scrapegraphai.com/pricing) for the full breakdown.
