...
| `allowedTypes`    | `string[]`    | No       | Non-empty MIME allowlist. All supported types, including PDF, are allowed by default |
| `processors`      | `object[]`    | No       | PDF settings, e.g. `[{ type: "pdf", maxPages: 10 }]`; default cap `25`, `-1` means unlimited |
| `fetchConfig`     | `FetchConfig` | No       | Fetch configuration                                                 |
