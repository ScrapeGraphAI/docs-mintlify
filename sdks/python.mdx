...
| `allowed_types`     | `list[str]`   | No       | Non-empty MIME allowlist. All supported types, including PDF, are allowed by default |
| `processors`        | `list[PdfProcessor]` | No | PDF settings such as `PdfProcessor(max_pages=10)`; default cap `25`, `-1` means unlimited |
| `fetch_config`      | `FetchConfig` | No       | Fetch configuration                                                       |
