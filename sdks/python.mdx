...
| `processors`        | `list[PdfProcessor]` | No | Omit for the 25-page PDF cap. `PdfProcessor()` also defaults to 25; set `max_pages` only to override it, or use `-1` for unlimited |
| `fetch_config`      | `FetchConfig` | No       | Fetch configuration                                                       |
