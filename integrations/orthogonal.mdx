...
The TypeScript SDK wraps Orthogonal's `Run API` and reads `ORTHOGONAL_API_KEY` from the environment.
...
orth login        # paste your orth_live_ key
```

```bash
# Discover ScrapeGraph endpoints
orth search "web scraping"
# → scrapegraph     ScrapeGraph AI  (6 endpoints)
...
Orthogonal also ships an MCP server so Claude, Cursor, OpenClaw, or any MCP-compatible client can call ScrapeGraph directly without writing glue code. Setup:

```bash
npx @orth/mcp install
```

Once installed the agent gets `search_apis`, `get_details`, `run_api`, `get_code`, and `list_apis` tools — calling `run_api` with `{ api: "scrapegraph", path: "/v1/smartscraper", body: {...} }` runs the same call as the SDK example above.
